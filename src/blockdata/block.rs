...
use hash_types::{Txid, BlockHash, MerkleRoot, WitnessCommit};
...
impl Block {
        ...
        if self.txdata.len() > 0 {
            ...
            if coinbase.is_coin_base() {
                ...
                if let Some(pos) = coinbase.output.iter()
                    ...
                    let commitment = WitnessCommit::from_slice(&coinbase.output[pos].script_pubkey.as_bytes()[6..38]).unwrap();
    ...
    pub fn compute_witness_commitment (witness_root: &MerkleRoot, witness_reserved_value: &[u8]) -> WitnessCommit {
        let mut encoder = WitnessCommit::engine();
        witness_root.consensus_encode(&mut encoder).unwrap();
        encoder.input(witness_reserved_value);
        WitnessCommit::from_engine(encoder)
    }
