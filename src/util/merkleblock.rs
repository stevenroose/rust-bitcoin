...
use hashes::Hash;
use hash_types::{Txid, MerkleRoot, MerkleBranch};
...
impl PartialMerkleTree {
    ...
    pub fn extract_matches(
        ...
        indexes: &mut Vec<u32>,
    ) -> Result<MerkleRoot, MerkleBlockError> {
        matches.clear();
        indexes.clear();
        ...
        Ok(MerkleRoot::from_inner(hash_merkle_root.into_inner()))
    }
    ...
    fn calc_hash(&self, height: u32, pos: u32, txids: &[Txid]) -> MerkleBranch {
        if height == 0 {
            // Hash at height 0 is the txid itself
            MerkleBranch::from_inner(txids[pos as usize].into_inner())
        } else {
            // Calculate left hash
    ...
    fn traverse_and_extract(
        ...
        matches: &mut Vec<Txid>,
        indexes: &mut Vec<u32>,
    ) -> Result<MerkleBranch, MerkleBlockError> {
    ...
    fn parent_hash(left: MerkleBranch, right: MerkleBranch) -> MerkleBranch {
        let mut encoder = MerkleBranch::engine();
        left.consensus_encode(&mut encoder).unwrap();
        right.consensus_encode(&mut encoder).unwrap();
        MerkleBranch::from_engine(encoder)
    }
}
...
impl Decodable for PartialMerkleTree {
    fn consensus_decode<D: io::Read>(mut d: D) -> Result<Self, encode::Error> {
        ...
        let hashes: Vec<MerkleBranch> = Decodable::consensus_decode(&mut d)?;
...
mod tests {
    ...
    fn pmt_tests() {
        ...
        for num_tx in tx_counts {
            ...
            for att in 1..15 {
                ...
                assert_ne!(merkle_root_2, MerkleRoot::default());
    ...
    impl PartialMerkleTree {
        ...
        fn damage(&mut self, rng: &mut ThreadRng) {
            ...
            hashes[n] = MerkleBranch::from_slice(&hash).unwrap();
        }
    }
