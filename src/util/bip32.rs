...
impl FromStr for ChildNumber {
    ...
    fn from_str(inp: &str) -> Result<ChildNumber, Error> {
        ...
        let nb: u32 = if last.is_some() && (last.unwrap() == '\'' || last.unwrap() == 'h') {
            hardened = true;
            inp[0..inp.len() - 1].parse()
        } else {
            inp.parse()
        }.map_err(|_| Error::InvalidChildNumberFormat)?;
        match hardened {
            true => Ok(ChildNumber::from_hardened_idx(nb)),
            false => Ok(ChildNumber::from_normal_idx(nb)),
        }
    }
}
...
pub fn parse_derivation_path(path: &str) -> Result<Vec<ChildNumber>, Error> {
    ...
    let mut childnumbers = Vec::new();
    for p in parts {
        childnumbers.push(p.parse()?);
    }
    Ok(childnumbers)
}
